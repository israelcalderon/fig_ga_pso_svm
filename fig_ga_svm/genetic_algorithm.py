...
POPULATION_SIZE = 220 # Cuántos individuos hay en cada generación.
NUM_GENES = 5         # Cada individuo se compone de 3 números.
MUTATION_RATE = 0.35  # Probabilidad de que un gen mute (10%).
STAGNATION_MUTATION_RATE = 0.65
TOURNAMENT_SIZE = 10   # Número de individuos que compiten en la selección.
NUM_GENERATIONS = 250 # Número de ciclos que correrá el algoritmo.
ELITISM_COUNT = 2     # Número de mejores individuos que pasan directo a la siguiente generación.
...
def create_population() -> list[tuple[float]]:
...
def roulette_wheel_selection(population: list[tuple]) -> tuple:
    total_fitness = sum([i[1] for i in population])
    population = sorted(population, key=lambda ind: ind[1])
    probabilities = sorted([i[1]/total_fitness for i in population])
    cumulative_probabilities = [0] * len(probabilities)

    for i, val in enumerate(probabilities):
        if i > 0:
            cumulative_probabilities[i] = cumulative_probabilities[i - 1] + val
        else:
            cumulative_probabilities[i] = val

    best_index = (len(probabilities) - 1,)
    selection_point = random.random()
    selection = next(
        filter(lambda i: i[1] > selection_point, enumerate(cumulative_probabilities)),
        best_index)[0]

    winner = population[selection]
    return winner[0]
...
def mutacion(individuo)-> tuple[Any]:
